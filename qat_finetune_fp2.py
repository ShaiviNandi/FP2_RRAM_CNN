...
so the crossbar benchmark can be re-run on them immediately to check whether
...
def load_checkpoint_into(model, path):
    ...
    if missing or unexpected:
        ...
        print("         (conv1/fc in that list means the checkpoint's arch "
...
def main():
    ...
    ap.add_argument("--num-classes", type=int, default=0,
                    help="Classifier width. 0 = infer from the dataset (10/100 for CIFAR, "
                         ...
                         "a silently-dropped fc means benchmarking a random classifier.")
    ap.add_argument("--save-fp32-checkpoint", default=None,
                    help="Write the FP32 master weights BEFORE QAT starts (i.e. right after "
                         ...
                         "the CONTROL checkpoint. Without it benchmark_resnet18.py cannot run "
                         "on the same architecture pre-QAT, and any SNR/utilization improvement "
                         "observed post-QAT is confounded with the change of model and dataset.")

    ap.add_argument("--pretrain-epochs", type=int, default=0,
                    help="Train FP32 (quantizer off) for this many epochs first. Use when "
