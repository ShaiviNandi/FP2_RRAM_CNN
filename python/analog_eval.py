...
and "the analog noise floor binds, B=32 is correct". Both are publishable; you
do not get to guess which.
...
def adc_sweep(ckpt, block, args, device):
    ...
    is what tells you where the knee is."""
...
def variability_sweep(ckpt, block, args, device):
    ...
    Three curves against sigma:
      ...
      blind        calibration from the NOMINAL programmed conductances, which
                   is what you can actually build: the compiler knows what it
    ...
    for sigma in sigmas:
        ...
        rows.append(dict(block_size=block, sigma=sigma, **acc))
