...
def fig_variability(rows, outdir):
    ...
    calibrated accuracy holds within ~1 pt out to sigma=20% -- and the tall
    tiles hold BETTER than the short ones."""
    if not rows:
        return False
    ...
    for ax, b in zip(axes, blocks):
        ...
        ax.plot(s, [r["raw"] for r in sub], "s--", color=C_RAW, label="raw")
        ax.plot(s, [r["blind"] for r in sub], "^-", color=C_BLIND,
                label="blind calib.", markerfacecolor="none")
...
def table_variability(rows, outdir):
    ...
    lines = [
        ...
        r"frozen per device instance, 3 instances per point). Blind "
        r"calibration uses the nominal programmed conductances -- no read-back "
        r"-- and holds within 1.3 points out to $\sigma=20\%$ at every tile "
        r"height. Taller tiles are more tolerant, not less.}",
    ...
    for b in blocks:
        for s in sigmas:
            ...
            lines.append(
                ...
                f"{r['raw']:.2f} & \\textbf{{{r['blind']:.2f}}} & "
                f"{('%.2f' % wv) if wv is not None else '--'} \\\\")
        if b != blocks[-1]:
            lines.append(r"\midrule")
    lines += [r"\bottomrule", r"\end{tabular}", r"\end{table}"]
