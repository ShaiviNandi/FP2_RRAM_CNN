...
labels each one so they never get conflated:
  ...
  [ASSUM] A literature-typical value for a technology this script has no
          ...
          magnitude, NOT measurements, and NOT specific to your PDK.
          Every one is a module-level constant you can override from the
          CLI (--set NAME=VALUE) or by editing this file.

An [ASSUM]-derived area number is a sanity check on feasibility, not a tapeout
estimate. If you have a real PDK, replace TECH_F_NM, CELL_AREA_F2, and the ADC
constants with your numbers and the whole report becomes defensible.
...
tension your SNR analysis already found from the other side -- larger M gives
...
# =============================================================================
# ASSUMPTIONS -- every one of these is a placeholder you should replace with
...
ASSUMPTIONS = {
    ...
    # ---- input DAC / wordline driver (per row) ----------------------------
    "DAC_BITS": 4.0,
    "DAC_AREA_MM2": 0.0004,
    ...
    # ---- operating point (must match what you simulated) -------------------
    "R_SENSE_OHM": 20.0,
    "V_READ_V": 0.1,
    ...
    # --power-csv is absent. Replace with your own measured mean if you re-run
...
def area_model(tile_m, tile_k, n_tiles_physical):
    ...
    n_adc = 2 * tile_k / ASSUMPTIONS["COLS_PER_ADC"]   # both bitline polarities
    adc_um2 = n_adc * ASSUMPTIONS["ADC_AREA_MM2"] * 1e6
    dac_um2 = tile_m * ASSUMPTIONS["DAC_AREA_MM2"] * 1e6
    psum_um2 = tile_k * ASSUMPTIONS["PSUM_ADDER_AREA_MM2"] * 1e6

    tile_um2 = array_um2 + adc_um2 + dac_um2 + psum_um2
    return dict(
        cells_per_tile=cells_per_tile,
        array_um2=array_um2, adc_um2=adc_um2, dac_um2=dac_um2, psum_um2=psum_um2,
        tile_um2=tile_um2,
...
def rollup(layers, power_by_layer, tile_m, tile_k, verbose=True, physical_tiles=0):
    ...
    the model telling you the assumption does not survive contact with a real
    die, not a bug.
    ...
    return dict(per_layer=per_layer, area=area,
...
def print_report(R, tile_m, tile_k, layers):
    ...
    print(f"  per tile: array {A['array_um2']:.0f} um2 ({100*A['array_frac']:.1f}%)   "
          ...
          f"DAC {A['dac_um2']:.0f} um2   psum {A['psum_um2']:.0f} um2")
    ...
    if R.get("reuse_factor", 1.0) > 1.0:
        ...
        print(f"  EXCLUDES tile reprogramming energy/time between passes -- with "
              f"{R['reuse_factor']:.0f} reloads that term is likely dominant.")
    else:
...
def sweep_m(layers, power_by_layer, tile_k, m_values=(8, 16, 32, 64, 128, 256),
    ...
    print("Energy/MAC falls with tile_M because the ADC's fixed per-column cost amortizes "
          "over more rows. Your SNR data pushes the other way (taller columns load the "
          "bitline harder). The tile size you want is where those curves cross, which is "
...
def main():
    ...
    if args.power_csv:
        ...
        print("WARNING: no --power-csv. The array term falls back to an "
              ...
              "is anchored to simulation but not to YOUR configuration. Pass "
