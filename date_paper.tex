...
\emph{Area denominators.} Tile-level and chip-level shares are not
interchangeable. The 55.2\% ADC share quoted here is of tile area --- array,
converters, input drive and partial-sum accumulation. The independent tool
reports 19.2\% of \emph{chip} area for the same block, the difference being
interconnect, global buffer and pooling, which the tile-level denominator
excludes. Its full breakdown accounts for 95.3\% of chip area, the 4.7\%
residual being global blocks it does not print separately.
